...
"javascript 부모 div 이벤트 접근" 검색
https://choonse.com/2022/02/02/789/

parentElement 프로퍼티 사용해서 버튼의 부모 div를 찾는다

submit하면 newTodo에 id를 랜덤으로 부여하게끔 해두어서 그에 해당하는
id값을 찾을 수 있다.

remove() 함수를 해용하여 div자체를 삭제해버려서 화면에서 삭제

문제는 화면에서는 삭제되었지만 LocalStorage에는 그대로 남아있다

이걸 지우려면..

방금 지운 div의 id와 일치하는 요소를 배열에서 지우고 남은 애들만 남기기

배열에서 지운다 => filter로 배열 요소를 하나씩 탐색해서

item의 id와 방금 지운 todo의 id가 일치하지 않으면 그대로 둔다

일치하면 조건을 만족하니 제외해버리기

filter((item) => item.id === todo.id) >> 일치하면 놔둠
filter((item) => item.id !== todo.id) >> 일치하지 않으면 놔둠
...
- new Date -> get 함수들
  month는 +1을 해줘야 한다. 0은 1월, 1은 2월...
  day는 0부터 정수를 반환해서 배열을 직접 만들어 매핑했다
